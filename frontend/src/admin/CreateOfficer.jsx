...
function CreateOfficer({ backendUrl }) {
  ...
  const register = async() => {
    try{
        ...
        const res = await axios.post(`${backendUrl}/auth/register`, {name: name, email: mail, password: password, officer: true})
        if(res.data?.success){
            showAlert('success', `Registration successful with ID ${res.data.idn}`);
        }
    } catch(err){
        console.error(err);
  ...
  <div className="fixed bottom-5 left-1/2 transform -translate-x-1/2 w-[420px] animate-popIn">
    <Alert type={alert.type} message={alert.message} />
      ...
      <div className="form">
        <div className="header flex text-sm flex-col">Register The Officer
            <br />
        ...
        <div className="inputs">
          ...
          <button className="sigin-btn" type="submit" onClick={register}>
            {'Register'}
          </button>
